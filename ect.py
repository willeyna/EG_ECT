...
def ecs_matrix(G):
    ...
    # compute E.C. seq along theta0 using subgraph generated by height function
    for i in range(0, len(eta)):
        cs_G = G.subgraph(eta[:i+1])
        phi[i] = len(cs_G.nodes) - len(cs_G.edges)

    # set up [E]
    E = np.zeros([len(crit), len(G.nodes())])
    E[0] = phi

    # iterate through crit angles in sorted increasing order
